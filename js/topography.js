...
// Vanilla JS port of react-bits' "Topography" background effect
// (originally built on React + the ogl WebGL micro-library — same
// dependency as the Ferrofluid effect it replaces here, so no new
// library needed).
// Ported faithfully from react-bits (Backgrounds/Topography), including
// the WebGL2 fragment shader, the "bezier field" contour-line math, and
// the intersection/visibility-based pause behavior from the original.
// Recolored per request: low #2457FF, high #DFF7FF, with mid computed
// as the midpoint blend of the two (the source component only exposed
// two colors here, so mid is an interpolated in-between tone rather
// than an arbitrary third hue).
//
// NOTE: like beams.js and ferrofluid.js before it, this file is kept for
// reference/version history but inlined directly into index.html rather
// than loaded via <script type="module" src="...">, since Chromium blocks
// module scripts loaded from local file:// paths.
...
function initTopography(container, options = {}) {
  const {
    lowColor = "#2457FF",
    midColor = "#82A7FF",
    highColor = "#DFF7FF",
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  try {
    ...
    console.warn("Topography background could not initialize:", err);
  }
});
